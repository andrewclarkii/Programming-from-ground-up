...
#          converted to uppercase. Program is
#          able to either operate on command-line
#          arguments or use STDIN or STDOUT based
#          on the number of command-line arguments
#          specified by ARGC. It also contains 
#          checks the results of each system call,
#          and prints out an error message to STDOUT
#          when it occurs
...
# first step:  as --32 -o toupper-4.o toupper-4.s
# second step: ld -melf_i386 -o toupper-4 toupper-4.o
...
error_message:          .ascii "Cannot open file for write, or file does not exist\n"
                        .set error_message_size, . - error_message
...
### INITIALIZE PROGRAM ###
	...
	open_files:
		...
		cmp	$1, %eax		# do we have any arguments or not 
		jg	open_fd_in		# if we have arguments then open a file

		movl	$STDIN, ST_FD_IN(%ebp)	# put STDIN number in input file descriptor
		movl	$STDOUT, ST_FD_OUT(%ebp) # put STDOUT number in output file descriptor
                jmp	read_loop_begin
...
### READ IN A BLOCK FROM THE INPUT FILE ###
		...
		mov	ST_FD_IN(%ebp), %ebx		# input file descriptor
...
### WRITE THE BLOCK OUT TO THE OUTPUT FILE ###
		...
		mov	$SYS_WRITE, %eax		# 
		mov	ST_FD_OUT(%ebp), %ebx		# output file descriptor
...
### EXIT ###
	...
	show_error:
		...
		push	%eax
		mov     $error_message, %ecx            # display string
                mov     $error_message_size, %edx       # string size
